 ...
 *
 * At every time unit the ready process with the smallest remaining burst
 * is selected. A newly arrived process can preempt the running one if its
 * burst is shorter.
 *
 * Compile & Run:
 *   javac SRT.java
...
public class SRT {
    ...
    //  SRT Simulation
    // ─────────────────────────────────────────────
    static List<GanttBlock> simulate(List<Process> procs) {
        List<Process>    ps    = deepCopy(procs);
        List<GanttBlock> gantt = new ArrayList<>();
        int t = 0, done = 0;
        int total = ps.stream().mapToInt(p -> p.burstTime).sum()
                  + ps.stream().mapToInt(p -> p.arrivalTime).max().orElse(0) + 2;

        while (done < ps.size() && t < total) {

            // pick the ready process with the shortest remaining time
            Process best = null;
            for (Process p : ps) {
                if (p.arrivalTime <= t && p.remainingTime > 0) {
                    if (best == null
            ...
            if (best == null) { t++; continue; }   // CPU idle

            if (best.startTime == -1) best.startTime = t;

            // extend last Gantt block or open a new one
            if (!gantt.isEmpty() && gantt.get(gantt.size() - 1).id.equals(best.id)) {
                gantt.get(gantt.size() - 1).end = t + 1;
            } else {
                gantt.add(new GanttBlock(best.id, t, t + 1));
            }

            best.remainingTime--;
            t++;
            if (best.remainingTime == 0) { best.finishTime = t; done++; }
        }
    ...
    public static void main(String[] args) {
        ...
        String title = "SRT \u2014 Shortest Remaining Time (Preemptive)";
        ...
        List<GanttBlock> gantt = simulate(procs);
